...
<script lang="ts" generics="TData">
  ...
  let { row }: { row: Row<ReservationPageTable> } = $props();
...
<AlertDialog.Root bind:open>
  <AlertDialog.Trigger class=" h-full rounded-full p-2 transition-all hover:bg-primary">
    <FileCheck class="h-4 w-4 text-destructive" />
  </AlertDialog.Trigger>
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description>
        ...
        <strong>{row.original.event_name}</strong> from our database.
      </AlertDialog.Description>
    </AlertDialog.Header>
    <AlertDialog.Footer>
